...
public class GearDataStore implements IGearDataStore {

    private final DataStore dataStore;
    @Getter
    IGearConnection connect;
    /**
     * 默认要素类的名称（取第一个）
     */
    private String defaultFeatureClassName;

    public GearDataStore(IGearConnection connect) throws Exception {
    ...
    @Override
    public IGearFeatureClass readFeatureClass(String featureClassName) throws IOException {
        SimpleFeatureSource featureSource = dataStore.getFeatureSource(featureClassName);
        return new GearFeatureClass(featureSource);
    }

    @Override
    public IGearFeatureClass readFeatureClass() throws IOException {

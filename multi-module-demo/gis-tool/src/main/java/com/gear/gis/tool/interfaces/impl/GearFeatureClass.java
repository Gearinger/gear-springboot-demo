...
import com.gear.gis.tool.common.FeatureClassUtil;
import com.gear.gis.tool.common.GeometryUtils;
 ...
 * @date 2022/03/03
 */
public class GearFeatureClass implements IGearFeatureClass {
     ...
     * @throws Exception 异常
     */
    public IGearFeatureClass readShp(String shpFile) throws Exception {
     ...
     */
    @Override
    public Stream<IGearFeature> getGearFeaturesStream() {
        throw new NotImplementedException();
    }

    @Override
    public IGearFeature getFeature(String fieldName, Object fieldValue) {
        for (IGearFeature gearFea : this) {
    ...
    @Override
    public IGearFeature buildFeature(Geometry geometry, Map<String, Object> fields) {
    ...
    @Override
    public boolean addOneFeature(IGearFeature feature) throws IOException {
    ...
    @Override
    public boolean addFeatures(IGearFeatureClass featureClass) throws IOException {
        this.addFeatures(featureClass, null);
        return true;
    }

    @Override
    public boolean addFeatures(List<IGearFeature> featureList) throws IOException {
        ...
        try (FeatureWriter<SimpleFeatureType, SimpleFeature> featureWriterAppend = dataStore.getFeatureWriterAppend(schema, Transaction.AUTO_COMMIT)) {
            for (IGearFeature feature : featureList) {
    ...
    @Override
    public boolean addFeatures(IGearFeatureClass featureClass, Map<String, String> fieldNameMaps) throws IOException {
        ...
        String schema = this.featureCollection.getSchema().getName().getLocalPart();
        try (FeatureWriter<SimpleFeatureType, SimpleFeature> featureWriterAppend = dataStore.getFeatureWriterAppend(schema, Transaction.AUTO_COMMIT)) {
            for (IGearFeature feature : featureClass) {
    ...
    private void writeFeature(FeatureWriter<SimpleFeatureType, SimpleFeature> featureWriterAppend, IGearFeature fea, Map<String, String> fieldNameMaps) {
        try {
            SimpleFeature targetFeature = featureWriterAppend.next();
    ...
    @Override
    public IGearFeatureClass intersect(IGearFeatureClass featureClass) throws ExecutionException, InterruptedException, IOException {
        ...
        IGearFeatureClass memory = FeatureClassUtil.buildMemoryFeatureClass("memory",
                this.featureCollection.getSchema().getCoordinateReferenceSystem(),
                fieldNameList);
        // 划分区域
        List<IGearFeatureClass> sourceFeatureClassList = new ArrayList<>();
        List<IGearFeatureClass> targetFeatureClassList = new ArrayList<>();
        if (true) {
            // 未实现，没找到合适的实现方法
        ...
        for (int i = 0; i < widthScale; i++) {
            for (int j = 0; j < heightScale; j++) {
                ...
                IGearFeatureClass intersect1 = new GearFeatureClass(tempSource).intersect(new GearFeatureClass(tempTarget));
            }
        }
        ...
        for (int i = 0; i < sourceFeatureClassList.size(); i++) {
            IGearFeatureClass source = sourceFeatureClassList.get(i);
            IGearFeatureClass target = targetFeatureClassList.get(i);
            if (source == null || target == null) {
                continue;
            }
            IGearFeatureClass tempIntersect = source.intersect(target);
    ...
    @Override
    public IGearFeatureClass erase(IGearFeatureClass featureClass) {
        SimpleFeatureCollection erase = FeatureClassUtil.erase(this.featureCollection, featureCollection);
